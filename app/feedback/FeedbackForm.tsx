...
export function FeedbackForm() {
  ...
  return (
    <div className="min-h-[calc(100vh-116px)] pb-16 bg-gradient-radial from-emerald-950 to-slate-950">
      {!submitted &&
        <form id="newpost" action={subHandler} className="flex justify-center pt-32 px-6">
          <div className="grid grid-cols-1 grid-flow-row auto-rows-auto gap-6 w-full max-w-2xl">
            ...
            <div className="max-w-2xl w-full h-10 flex justify-end space-x-5">
              <button type="submit" className="outline outline-2 outline-slate-700 rounded-md p-2 text-slate-400 bg-slate-50 bg-opacity-5">Submit</button>
            </div>
          </div>
        </form>
      }
      {submitted &&
        <div className="flex justify-center pt-80">
          <div className="grid grid-cols-1 grid-rows-2 gap-3">
            <header className="text-3xl">Your input is appreciated. Thank you!</header>
            <header className="flex justify-end">Redirecting in {redirectSeconds}</header>
          </div>
        </div>
